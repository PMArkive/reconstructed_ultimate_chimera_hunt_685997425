...
function GM:UpdateAnimation(ply, velocity, maxseqgroundspeed)
	...
	if (len2d > .5) then
		rate = ((len2d * .8 ) / maxseqgroundspeed)
	end
	...
	if (!ply:IsUC()) then
		...
		if (pp > .9 || pp < .1 || len2d > 0) then
			...
			if (CLIENT) then
				...
				ply.SmoothBodyAnglesCL = math.ApproachAngle(ply.SmoothBodyAnglesCL, eye.y, 65) -- Used to be ply.SmoothBodyAnglesCL = math.ApproachAngle(ply.SmoothBodyAnglesCL, eye.y, num); Look here to fix animations!!!
	...
	if (ply:GetNWBool("PlaybackOV")) then
