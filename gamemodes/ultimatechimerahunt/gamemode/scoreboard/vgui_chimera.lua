...
surface.CreateFont("UCH_UltimateChimera", {font = "coolvetica", size = ScreenScale(7), weight = 400, antialias = true, additive = false});
...
function PANEL:Init()
	...
	self.UCText = vgui.Create("DLabel", self);
	self.UCText:SetFont("UCH_UltimateChimera");
	self.UCText:SetText("Ultimate Chimera:");
	
	self.UCImage = vgui.Create("DImage", self);
	self.UCImage:SetImage("uch/scoreboard/chimera");
	
	self.VoteBtn = vgui.Create("DButton", self);
	self.VoteBtn:SetText("Vote New UC");
	
	function self.VoteBtn:DoClick()
		RunConsoleCommand("uch_vote_uc");
	end
	function self.VoteBtn:Paint()
		
		local alpha = 80;
		local ply = LocalPlayer();
		if (ply:Team() == TEAM_PIGS && !ply:GetNWBool("UC_Voted") && Playing()) then
			alpha = 255;
		end
	...
	self.plyName:SetFont("Default");
	self.plyPing:SetFont("Default"); -- Font: ScoreboardText
...
function PANEL:UpdatePlayerData()
	...
	self.VoteBtn:SetCursor(((LocalPlayer():GetNWBool("UC_Voted", false) || !Playing()) && "arrow") || "hand");
