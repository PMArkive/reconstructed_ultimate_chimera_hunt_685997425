...
function PANEL:Init()
	...
	function self.List:Paint()
		...
		--Scoreboard Player Box Seasonal Recolor
		if (season == 1) then
			draw.RoundedBox(10, 0, 0, w, self:GetTall(), Color(210, 132, 0, 255));
		elseif (season == 2) then
			draw.RoundedBox(10, 0, 0, w, self:GetTall(), Color(184, 207, 241, 255));
		else
	...
	local function paintBtn(btn, t)
		...
		--Sidebar Seasonal Recolor
		if (season == 1) then
			slideclr = Color(240, 107, 0, 255);
		else
			slideclr = Color(247, 217, 219, 255);
		end
...
function PANEL:PerformLayout(w, h)
	...
	for k, v in pairs(list) do
		if (!IsValid(v)) then errorz = true return end
